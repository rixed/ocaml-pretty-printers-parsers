...
struct
  ...
  let array (ppp : 'a P.t) : 'a array P.t =
    P.seq "array" "[|" "|]" ";" Array.fold_left (fun l -> Array.of_list (List.rev l)) ppp
  (*$= array & ~printer:id
  ...
  module Tuple = struct
    ...
    let tuple2 (p1 : 'a P.t) (p2 : 'b P.t) : ('a * 'b) P.t =
      first p1 +- sep ++ last p2

    let tuple3 (p1 : 'a P.t) (p2 : 'b P.t) (p3 : 'c P.t) : ('a * 'b * 'c) P.t =
      first p1 +- sep ++ p2 +- sep ++ last p3 >>:
        ((fun (v1,v2,v3) -> (v1,v2),v3),
         (fun ((v1,v2),v3) -> v1,v2,v3))

    let tuple4 (p1 : 'a P.t) (p2 : 'b P.t) (p3 : 'c P.t) (p4 : 'd P.t) : ('a * 'b * 'c * 'd) P.t =
      first p1 +- sep ++ p2 +- sep ++ p3 +- sep ++ last p4 >>:
        ((fun (v1,v2,v3,v4) -> ((v1,v2),v3),v4),
         (fun (((v1,v2),v3),v4) -> v1,v2,v3,v4))

    let tuple5 (p1 : 'a P.t) (p2 : 'b P.t) (p3 :'c P.t) (p4 : 'd P.t) (p5 :'e P.t) : ('a * 'b * 'c * 'd * 'e) P.t =
  ...
  let record x = P.record "{" "}" "=" ";" groupings delims P.identifier x
  let (<->) x y = P.sequence "; " x y
  let field ?default name x = P.field "=" "; " ": " ?default name x

  let union x = P.union "" "" "" groupings delims P.identifier x
